...
export default function DashboardPage() {
  ...
  const avgNps =
    surveys.filter((s) => s.avgNps !== null).length > 0
      ? Math.round(
          surveys.filter((s) => s.avgNps !== null).reduce((s, sv) => s + (sv.avgNps ?? 0), 0) /
            surveys.filter((s) => s.avgNps !== null).length
        )
      : 0
  ...
  const recentActivity = [
    ...
    ...(complianceBlocked > 0
      ? [{ icon: "ShieldAlert", text: `${complianceBlocked} expert${complianceBlocked === 1 ? "" : "s"} compliance-blocked -- requires review`, time: "Action needed" }]
      : []),
  ]
